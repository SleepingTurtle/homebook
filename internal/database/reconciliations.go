...
func (db *DB) ListReconciliations() ([]models.BankReconciliation, error) {
	rows, err := db.Query(`
		SELECT id, date(statement_date), strftime('%m-%d-%Y', statement_date),
			   starting_balance, ending_balance, status, notes, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.StatementDate, &r.StatementDateDisplay,
			&r.StartingBalance, &r.EndingBalance, &r.Status, &r.Notes,
...
func (db *DB) CreateReconciliation(r models.BankReconciliation) (int64, error) {
	result, err := db.Exec(`
		INSERT INTO bank_reconciliations (statement_date, starting_balance, ending_balance, status, notes)
		VALUES (?, ?, ?, ?, ?)
	`, r.StatementDate, r.StartingBalance, r.EndingBalance, r.Status, r.Notes)
	if err != nil {
...
func (db *DB) UpdateReconciliation(r models.BankReconciliation) error {
	_, err := db.Exec(`
		...
		SET statement_date = ?, starting_balance = ?, ending_balance = ?, status = ?, notes = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`, r.StatementDate, r.StartingBalance, r.EndingBalance, r.Status, r.Notes, r.ID)
	if err != nil {
